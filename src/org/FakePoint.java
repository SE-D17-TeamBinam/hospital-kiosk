...
public class FakePoint {
  ...
  public FakePoint(int xCoord, int yCoord, String name, int id, ArrayList <Integer> new_neighbors, int floor){
    this.xCoord = xCoord;
    this.yCoord = yCoord;
    this.name = name;
    this.id = id;
  ...
  /**
   * Creates a new real point with all the same parameters EXCEPT WITHOUT THE NEIGHBORS, THAT IS ON YOU
   * @return The Real Point Equivalent (w/o Neighbors)
   */
  Point toRealPoint(){
    Point ret = new Point(this.xCoord, this.yCoord, this.name, this.id, new ArrayList<Point>(), this.floor);
    return ret;
  }
